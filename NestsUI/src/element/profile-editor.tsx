...
export function ProfileEditor({ onClose }: { onClose: () => void }) {
    ...
    const { system, publisher, signer } = useEventBuilder();
        ...
        return debounce(500, async () => {
            if (lud16) {
                try {
                    ...
                    setLud16Valid(false);
                    setInvalidLud16Message(formatMessage({
        ...
        async function nip05NostrAddressVerification(nip05Domain: string | undefined, nip05Name: string | undefined) {
            try {
                ...
                if (result) {
                    ...
                    setInvalidNip05AddressMessage(formatMessage({
    ...
    async function uploadFile() {
        try {
            ...
            if (file) {
                const rsp = await nostrBuildUpload(file, signer);
            ...
            <div className="flex flex-col gap-4">
                <div className="mx-auto relative flex items-center justify-center w-40 h-40 aspect-square rounded-full overflow-hidden">
                    <img
                        ...
                        src={picture}
                    />
                    <div className="flex items-center justify-center absolute w-full h-full opacity-0 hover:opacity-80 bg-foreground cursor-pointer" onClick={() => setNewAvatar()}>
