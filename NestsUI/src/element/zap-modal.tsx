...
export default function ZapFlow({ targets, onClose }: { targets: Array<ZapTarget>, onClose: () => void }) {
    ...
    return <Modal id="zap" onClose={onClose}>
        <div className="flex flex-col gap-4">
            ...
            {!invoice && <><div className="grid grid-cols-[max-content_auto_max-content] items-center select-none">
                <IconButton name="chevron" className="rounded-full aspect-square" onClick={() => setAmount(v => Math.max(inc, v - inc))} />
                ...
                <IconButton name="chevron" className="rounded-full aspect-square rotate-180" onClick={() => setAmount(v => Math.min(1_000_000, v + inc))} />
            </div>
                ...
                <PrimaryButton onClick={async () => {
                    if (signer && pubkey) {
                        ...
                        const res = await zapper.send(window.webln ? WebLnWallet : undefined, targets.map(a => ({ ...a, memo: comment })), amount);
                        if (!res[0].paid) {
                            setInvoice(res[0].pr)
                        }
                    }
