...
export default function ProfileCard({ participant, pubkey }: { participant: LocalParticipant | RemoteParticipant, pubkey: string }) {
  ...
  const menuItem = (icon: string, text: ReactNode, onClick: () => void, className?: string) => {
    return <div className={classNames("flex items-center gap-3 px-4 leading-10 hover:bg-foreground transition cursor-pointer select-none", className)} onClick={onClick}>
      <Icon name={icon} />
      <div>{text}</div>
    </div>
  }
  return (
    <div className="absolute z-10 bg-foreground-2 rounded-xl overflow-hidden flex flex-col font-medium w-max">
      ...
      {isAdmin && <>
        ...
        <hr className="mx-4" />
        {menuItem("minus-circle", "Ban user", () => {
