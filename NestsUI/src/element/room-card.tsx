...
export default function RoomCard({ event, inRoom, className, link, join }: { event: NostrEvent, inRoom?: boolean, className?: string, link?: boolean, join?: boolean }) {
    ...
    const inner = () => {
        ...
        return <div className={classNames("relative px-6 py-4 rounded-3xl flex flex-col gap-3", image ? "" : `bg-${color}`, { "pt-20": inRoom, "cursor-pointer": (link ?? true) || join }, className)} onClick={() => {
            if (join) {
                joinRoom();
            }
        }} style={styles}>
            {inRoom && event.pubkey === login.pubkey && <div className="absolute right-2 top-2">
            ...
            {editRoom && <Modal id="edit-room" onClose={() => setEditRoom(false)}>
    ...
    if ((link ?? true) && !join) {
        return <Link to={`/${NostrLink.fromEvent(event).encode()}`} state={{ event }}>
            {inner()}
        </Link>
    }
    return inner();
