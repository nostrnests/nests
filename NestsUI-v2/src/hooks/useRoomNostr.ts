 ...
 * scoped to the room's effective relay set (NIP-65 union room `relays` tag
 * union naddr relay hints).
 *
...
export function useRoomNostr() {
  ...
  const scoped = useMemo(() => {
    ...
    // `nostr` is an NPool (created in NostrProvider) — `.group()` returns a
    // sibling pool with explicit reqRouter/eventRouter targeting these URLs
    // and shared underlying connections.
    return (nostr as NPool).group(roomRelays);
